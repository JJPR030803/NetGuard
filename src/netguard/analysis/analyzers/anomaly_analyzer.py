...
class AnomalyAnalyzer(BaseAnalyzer):
    ...
    # ============================================================================
    # Reccomended port scanning threshold: 20 - 30
    def detect_port_scanning(
        ...
        Args:
            config: Port scan configuration (profile or PortScanConfig object)
            threshold: Override threshold (if config not provided)
            time_window: Override time window (if config not provided)
        ...
        Examples:
            ...
            #Custom config
            >>> custom_config = create_custom_config(threshold=25,time_window="10m")
            >>> results = analyzer.detect_port_scanning(config=custom_config)
        ...
        if isinstance(config, ScanProfile):
            ...
            scan_config = create_custom_config(
                ...
                time_window=time_window or self.DEFAULT_SCAN_CONFIG.time_window,
            )
        else:
            scan_config = self.DEFAULT_SCAN_CONFIG

        print(f"Using config: {scan_config}")
